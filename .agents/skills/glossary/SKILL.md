---
name: glossary
description: Interpret, introduce, rename, persist, transport, authorize, or publicly present Proctor domain terms and invariants. Load this before any other domain-sensitive skill.
---

# Proctor domain glossary

Use the canonical terms below when domain meaning is in scope. Before changing
a term, account for its code, persistence, transport, authorization, test, and
public-documentation consumers. Preserve the distinctions expressed by each
`Avoid` line; introduce a new term only when an existing term cannot carry the
meaning without ambiguity.

After changing a definition, regenerate and validate the public glossary from
`docs/site`:
