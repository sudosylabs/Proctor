...
Root [`CONTEXT.md`](../../CONTEXT.md) remains the only domain-language
authority. The public glossary and runtime lookup data are deterministic views
generated by the glossary module under `docs/site/glossary/`; do not edit the
generated MDX or TypeScript files directly.

Use `<Term id="academic-unit">Academic Unit</Term>` only on a deliberately
selected first occurrence where a reader benefits from an immediate definition.
The identifier is the lowercase, hyphenated canonical term. Term markup is
explicit rather than inferred so authors control density and meaning. It is
forbidden in headings, code, API paths, and illustrations, and the same term may
not be annotated twice on one page.

After changing `CONTEXT.md` or curated term markup, run `npm run
