...
type browserAuthenticationRow struct {
	...
	PasswordCredentialID         sql.NullString `db:"password_credential_id"`
	PasswordCredentialRevision   sql.NullInt64  `db:"password_credential_revision"`
...
user_id, authentication_method, authentication_provider_id, external_identity_id, password_credential_id, password_credential_revision, authentication_strength, authenticated_at,
...
func (s SQLBrowserAuthenticationStore) AuthenticateDesktopAuthorization(ctx context.Context, input *store.DesktopAuthorizationAuthentication) (*store.DesktopAuthorizationAuthenticationResult, error) {
	...
	return runSQLTransaction(ctx, s.GetMaster().Begin, "authenticate desktop authorization", func(ctx context.Context, tx *sqlxTxWrapper) (*store.DesktopAuthorizationAuthenticationResult, error) {
		...
		if activeAttempt {
			result, err := tx.Exec(ctx, `UPDATE browser_authentication_transactions
			   SET updated_at=terminal.at,state='denied',handle_hash=NULL,browser_proof_hash=NULL,state_hash=NULL,
			       ...
			       authentication_provider_id=NULL,external_identity_id=NULL,password_credential_id=NULL,password_credential_revision=NULL,authentication_strength=NULL,
		...
		// Fresh local proof gets one database authentication instant. Provider
		// history can only move back to that instant; Session history stays exact.
		authenticatedAt := model.TimeFromMillis(input.AuthenticatedAt)
		mfaCompletedAt := model.OptionalTimeFromMillis(input.MFACompletedAt)
		if source != nil {
			authenticatedAt, mfaCompletedAt = source.AuthenticatedAt, source.MFACompletedAt
		}
		freshPassword := source == nil && input.AuthenticationMethod == "password"
		// A User row lock above may outlast a previously checked source proof.
		// Recheck all source deadlines against this same transition instant.
		var authenticated browserAuthenticationRow
		err = tx.Get(ctx, &authenticated, `UPDATE browser_authentication_transactions
		   SET updated_at=authenticated.at,state='authenticated',user_id=?,authentication_method=?,
		       authentication_provider_id=NULLIF(?,''),external_identity_id=NULLIF(?,''),
		       password_credential_id=NULLIF(?,''),password_credential_revision=NULLIF(?,0),authentication_strength=?,
		       authenticated_at=CASE WHEN ? THEN authenticated.at WHEN ? THEN ?::timestamptz
		                             ELSE LEAST(?::timestamptz,authenticated.at) END,
		       mfa_completed_at=CASE WHEN ? THEN NULL WHEN ? THEN authenticated.at WHEN ? THEN ?::timestamptz
		                             ELSE LEAST(?::timestamptz,authenticated.at) END
		   ...
		   AND (? OR EXISTS (
		       SELECT 1 FROM sessions source JOIN session_credentials credential ON credential.session_id=source.id
		        WHERE source.id=? AND source.user_id=? AND source.client_type='web'
		          AND source.archived_at IS NULL AND source.revoked_at IS NULL
		          AND source.expires_at>authenticated.at AND source.idle_expires_at>authenticated.at
		          AND credential.id=? AND credential.kind='access'
		          AND credential.archived_at IS NULL AND credential.revoked_at IS NULL
		          AND credential.expires_at>authenticated.at))
		...
		if _, err = authenticated.model(); err != nil {
			return nil, err
		}
...
func (s SQLBrowserAuthenticationStore) ResetDesktopAuthorizationAccount(ctx context.Context, input *store.DesktopAuthorizationAccountReset) error {
	...
	result, err := s.GetMaster().Exec(ctx, `UPDATE browser_authentication_transactions
	   SET updated_at=reset.at,state='bound',user_id=NULL,authentication_method=NULL,authentication_provider_id=NULL,
	       external_identity_id=NULL,password_credential_id=NULL,password_credential_revision=NULL,authentication_strength=NULL,authenticated_at=NULL,mfa_completed_at=NULL
...
func (s SQLBrowserAuthenticationStore) Cancel(ctx context.Context, input *store.DesktopAuthorizationCancellation) error {
	...
	_, err := runSQLTransaction(ctx, s.GetMaster().Begin, "cancel desktop authorization", func(ctx context.Context, tx *sqlxTxWrapper) (struct{}, error) {
	   ...
	   SET updated_at = terminal.at, state = 'cancelled', handle_hash = NULL, browser_proof_hash = NULL,
	       ...
	       proposed_public_jwk=NULL,proposed_key_thumbprint=NULL,desktop_release=NULL,desktop_build_id=NULL,
	       desktop_platform=NULL,desktop_architecture=NULL,desktop_realtime_protocol=NULL,
	       user_id=NULL,authentication_method=NULL,authentication_provider_id=NULL,external_identity_id=NULL,password_credential_id=NULL,password_credential_revision=NULL,
...
// The caller holds the per-User Session lock through the terminal transition.
// Session reuse derives current assurance from durable state, never a previously
// resolved Principal. Direct password authentication instead supplies the exact
// credential revision verified before entering this transaction.
func resolveDesktopAuthenticationProof(ctx context.Context, executor sqlxExecutor, input *store.DesktopAuthorizationAuthentication) (*model.Session, error) {
	var source *model.Session
	if input.SourceSessionID.IsValid() {
		var row sessionRow
		if err := executor.Get(ctx, &row, `SELECT `+strings.Join(sessionSliceColumns(), ",")+`
		  FROM sessions JOIN session_credentials credential ON credential.session_id=sessions.id
		 WHERE sessions.id=? AND sessions.user_id=? AND sessions.client_type='web'
		   AND sessions.archived_at IS NULL AND sessions.revoked_at IS NULL
		   AND sessions.expires_at>clock_timestamp() AND sessions.idle_expires_at>clock_timestamp()
		   AND credential.id=? AND credential.kind='access'
		   AND credential.archived_at IS NULL AND credential.revoked_at IS NULL
		   AND credential.expires_at>clock_timestamp()`,
			input.SourceSessionID.String(), input.UserID.String(), input.SourceCredentialID.String()); err != nil {
			return nil, translateError("session", "desktop_authentication", err)
		}
		...
		input.AuthenticationProviderID = session.AuthenticationProviderID
		input.ExternalIdentityID = session.ExternalIdentityID
		input.AuthenticationStrength = session.AuthenticationStrength
		if err := requireDesktopAuthenticationPath(ctx, executor, input.AuthenticationMethod, input.AuthenticationProviderID, input.Capabilities); err != nil {
			return nil, err
		}
		if session.AuthenticationMethod == "password" {
			...
			input.PasswordProof = store.PasswordCredentialProof{ID: model.PasswordCredentialID(current.ID), Revision: current.Revision}
		}
	}
	if input.AuthenticationMethod == "password" {
		if err := requireCurrentPasswordProof(ctx, executor, input.UserID, input.PasswordProof); err != nil {
			return nil, err
		}
	}
	return source, nil
...
func validateDesktopAuthorizationAuthentication(input *store.DesktopAuthorizationAuthentication) error {
	...
	usingSession := !input.SourceSessionID.IsZero() || !input.SourceCredentialID.IsZero()
	if usingSession {
		if !input.SourceSessionID.IsValid() || !input.SourceCredentialID.IsValid() || input.PasswordProof != (store.PasswordCredentialProof{}) {
			return store.NewErrInvalidInput("browser_authentication_transaction", "source_session", nil)
		}
	} else if input.AuthenticationMethod == "password" {
		...
		return store.NewErrInvalidInput("browser_authentication_transaction", "password_proof", nil)
	}
	if input.AuthenticationMethod == "password" {
...
func (row browserAuthenticationRow) model() (*model.BrowserAuthenticationTransaction, error) {
	...
	value := &model.BrowserAuthenticationTransaction{
		...
		PasswordCredentialID:       model.PasswordCredentialID(row.PasswordCredentialID.String),
		PasswordCredentialRevision: row.PasswordCredentialRevision.Int64,
