...
func TestAccessPolicyStore(t *testing.T) {
	...
	storetest.TestAccessPolicyStore(t, persistence, storetest.AccessPolicySQLProbe{
		HoldAuthenticationPathFence: func(t *testing.T, ctx context.Context) (int, func()) {
			...
			if _, err = connection.ExecContext(ctx, `SELECT pg_advisory_lock(hashtextextended($1, 0))`, systemAdministratorAuthenticationPathLock); err != nil {
				_ = connection.Close()
				t.Fatal(err)
			}
			var once sync.Once
			return pid, func() {
				once.Do(func() {
					_, _ = connection.ExecContext(context.Background(), `SELECT pg_advisory_unlock(hashtextextended($1, 0))`, systemAdministratorAuthenticationPathLock)
					_ = connection.Close()
				})
			}
		},
