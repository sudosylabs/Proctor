...
func TestAffiliationStore(t *testing.T, ss store.Store) {
	...
	requireNoError(t, err)
	class := saveClass(t, ctx, ss, level.ID.String(), period.Id, "affiliation-class")
	...
	requireNoError(t, err)
	_, err = ss.ClassMember().Enroll(ctx, &model.ClassMember{ClassId: class.Id, UserId: enrolledUser.Id, StartAt: start})
	requireNoError(t, err)
