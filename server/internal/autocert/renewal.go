...
func (dr *domainRenewal) renew() {
	...
	lifecycleCtx, err := dr.m.lifecycleContext()
	if err != nil {
		dr.timer = nil
		return
	}
	ctx, cancel := context.WithTimeout(lifecycleCtx, 10*time.Minute)
	defer cancel()
	// TODO: rotate dr.key at some point?
	next, err := dr.do(ctx)
	if ctx.Err() != nil || dr.m.closed.Load() {
		dr.timer = nil
		return
	}
	if err != nil {
		next = time.Hour / 2
		next += time.Duration(dr.m.randomInt63n(int64(next)))
	}
	if dr.m.didRenewLoop != nil {
		dr.m.didRenewLoop(next, err)
	}
...
func (dr *domainRenewal) next(notBefore, notAfter time.Time) time.Duration {
	...
	jitter := dr.m.randomInt63n(int64(maxJitter))
