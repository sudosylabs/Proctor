...
const (
	...
	DesktopAuthorizationCallbackMaximumBytes = 1024
)
...
type BrowserAuthenticationTransaction struct {
	...
	PasswordCredentialID       PasswordCredentialID `json:"-"`
	PasswordCredentialRevision int64                `json:"-"`
...
func (t *BrowserAuthenticationTransaction) Validate() error {
	...
	if !t.hasValidPasswordProof() {
		return invalidModelError(where, "browser_authentication_transaction", "password_proof", "must match the authentication method", details)
	}
...
func (t *BrowserAuthenticationTransaction) hasNoDesktopAuthentication() bool {
	return t.UserID.IsZero() && t.AuthenticationMethod == "" && t.AuthenticationProviderID == "" &&
		t.PasswordCredentialID.IsZero() && t.PasswordCredentialRevision == 0 &&
...
func (t *BrowserAuthenticationTransaction) hasValidPasswordProof() bool {
	if t.AuthenticationMethod == "password" {
		return t.PasswordCredentialID.IsValid() && t.PasswordCredentialRevision > 0
	}
	return t.PasswordCredentialID.IsZero() && t.PasswordCredentialRevision == 0
}
...
func ValidateDesktopAuthorizationCallback(callback string) error {
	if len(callback) > DesktopAuthorizationCallbackMaximumBytes {
