...
func TestAcademicPeriodOpenAPIAgreesWithRuntime(t *testing.T) {
	...
	suite := openAPIAgreementSuite{
		Operations: []openAPIAgreementOperation{
			...
			{Key: "POST /api/v1/academic-periods", Auth: AuthPrincipalRequired, Idempotency: IdempotencyOptional, RequestBodyRef: "#/components/requestBodies/CreateAcademicPeriod", RequestSchema: "CreateAcademicPeriodRequest", SuccessStatus: "201", SuccessRef: "#/components/responses/AcademicPeriodCreated", SuccessSchema: "AcademicPeriodResponse", PublicErrorCodes: principalMutationContractCodes("request.invalid", "resource.not_found", "academic_period.invalid", "academic_period.conflict", "administration.unavailable", "idempotency.invalid_key", "idempotency.conflict", "idempotency.in_progress")},
		...
		Schemas: []openAPIAgreementSchema{
			{Name: "AcademicPeriodResponse", DTO: reflect.TypeOf(academicPeriodResponse{}), Required: []string{"revision", "id", "create_at", "update_at", "delete_at", "owner_type", "owner_id", "name", "display_name", "description", "start_at", "end_at"}},
			...
			{Name: "UpdateAcademicPeriodRequest", DTO: reflect.TypeOf(updateAcademicPeriodRequest{}), NonNullable: []string{"expected_revision"}},
		},
	}
