...
func TestExamOpenAPIAgreesWithRuntime(t *testing.T) {
	...
	suite := openAPIAgreementSuite{
		Operations: []openAPIAgreementOperation{
			...
			{
				Key: "PUT /api/v1/exams/{exam_id}/draft/policies/native", Auth: AuthPrincipalRequired, Idempotency: IdempotencyRequired,
				RequestBodyRef: "#/components/requestBodies/ConfigureExamDraftNativePolicy", RequestSchema: "ConfigureExamDraftNativePolicyRequest",
				SuccessStatus: "200", SuccessRef: "#/components/responses/ExamOK", SuccessSchema: "ExamResponse",
				PublicErrorCodes: principalMutationContractCodes("request.invalid", "resource.not_found", "exam.invalid", "exam.archived", "exam.draft.revision_conflict", "exam.draft.no_changes", "exam.unavailable", "idempotency.key_required", "idempotency.invalid_key", "idempotency.conflict", "idempotency.in_progress", "administration.unavailable"),
			},
			{
		...
		Schemas: []openAPIAgreementSchema{
			...
			{Name: "ConfigureExamDraftNativePolicyRequest", DTO: reflect.TypeOf(configureExamDraftNativePolicyRequest{}), Required: []string{"expected_draft_revision", "native"}},
			{Name: "NativeSecurityPolicy", DTO: reflect.TypeOf(model.NativeSecurityPolicy{}), Required: []string{"registry_digest", "baseline_id", "families"}},
			...
			{Name: "ExamDraftResponse", DTO: reflect.TypeOf(examDraftResponse{}), Required: []string{"exam_id", "title", "instructions_markdown", "policy", "browser_policy", "capacity", "updated_at", "revision", "resource_count", "has_starter_workspace"}},
			{Name: "BrowserPolicy", DTO: reflect.TypeOf(browserPolicyDocument{}), Required: []string{"enabled"}},
			{Name: "BrowserPolicyRule", DTO: reflect.TypeOf(browserPolicyRuleDocument{}), Required: []string{"rule_id", "origin", "path_prefix", "host_match", "allow_redirects", "blocked_navigation_outcome", "institution_http_exception"}},
			{Name: "ExamPolicySet", DTO: reflect.TypeOf(examPolicyResponse{}), Required: []string{"native", "connection_loss", "focus_loss"}},
