...
func TestDependencyPolicyRejectsForbiddenImports(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Execution host adapter cannot import SQL persistence", from: serverModule + "/executionhost", imported: serverModule + "/store/sqlstore"},
...
func TestDependencyPolicyAllowsInwardImports(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "Execution host adapter may import its port value contracts", from: serverModule + "/executionhost", imported: serverModule + "/store"},
