...
func TestMailRekeyHandlerReencryptsBoundPayloadAndCheckpoints(t *testing.T) {
	...
	envelope, err := oldSealer.Seal(secretseal.Binding{Purpose: mailDeliverySealingPurpose, Owner: deliveryID.String()}, []byte(`{"version":1,"secret":"payload"}`))
	if err != nil {
		t.Fatal(err)
	}
	...
	plaintext, err := sealer.Open(secretseal.Binding{Purpose: mailDeliverySealingPurpose, Owner: deliveryID.String()}, rotated)
...
func TestMailRekeyHandlerFailsClosedForUnavailableActiveKey(t *testing.T) {
	...
	deliveryID := model.NewMailDeliveryID()
	envelope, err := unavailableSealer.Seal(secretseal.Binding{Purpose: mailDeliverySealingPurpose, Owner: deliveryID.String()}, []byte(`{"version":1}`))
	if err != nil {
		t.Fatal(err)
	}
	encrypted, _ := json.Marshal(envelope)
