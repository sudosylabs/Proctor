...
func newAuthoringFixture(t *testing.T) authoringFixture {
	...
	service, err := NewAuthoring(persistence, memberships, users, mail, authorizer, auditor, outcomes, effects, effects, func() time.Time {
		return time.Date(2026, 8, 14, 8, 0, 0, 0, time.UTC)
	}, func() model.ExamID { return examID }, "https://institution.example")
	if err != nil {
		t.Fatal(err)
	}
	return authoringFixture{service: service, call: NewCall(testPrincipal(userID), model.RequestMetadata{}), unitID: unitID, examID: examID, userID: userID, order: &order, authorizer: authorizer, memberships: memberships, users: users, mail: mail, auditor: auditor, outcomes: outcomes, persistence: persistence, effects: effects}
}
