...
func (a *App) GetAcademicUnit(
	...
	if _, appErr := a.authorizePrincipalToAcademicUnit(
		ctx, principal, id, model.ActionAcademicUnitView, metadata,
	); appErr != nil {
		return nil, appErr
	}
	unit, err := a.Store().AcademicUnit().Get(ctx, id)
	if err != nil {
		return nil, administrationError("GetAcademicUnit", "academic_unit", err)
	}
	return unit, nil
}
...
func (a *App) ListAcademicUnits(
	...
	institution, err := a.Store().Institution().GetSingleton(ctx)
	if err != nil {
		return nil, administrationError("ListAcademicUnits.institution", "institution", err)
	}
	if parentID == "" {
		if _, appErr := a.authorizePrincipalToSystem(
			ctx, principal, model.ActionInstitutionManage, metadata,
		...
		ctx, principal, parentID, model.ActionAcademicUnitView, metadata,
	); appErr != nil {
		return nil, appErr
	}
	units, err := a.Store().AcademicUnit().ListChildren(ctx, institution.Id, parentID)
	if err != nil {
		return nil, administrationError("ListAcademicUnits", "academic_unit", err)
	}
	return units, nil
}
...
func (a *App) SearchAcademicUnits(
	...
	institution, err := a.Store().Institution().GetSingleton(ctx)
	if err != nil {
		return nil, administrationError("SearchAcademicUnits.institution", "institution", err)
	}
	limit = normalizeAdministrationLimit(limit)
	units, err := a.Store().AcademicUnit().Search(
		ctx, institution.Id, strings.TrimSpace(query), limit,
	)
	if err != nil {
		return nil, administrationError("SearchAcademicUnits", "academic_unit", err)
	}
	return units, nil
}
...
func (a *App) PatchAcademicUnit(
	...
	metadata model.RequestMetadata,
	id string,
	patch *model.AcademicUnitPatch,
) (*model.AcademicUnit, *model.AppError) {
	resource, appErr := a.authorizePrincipalToAcademicUnit(
		ctx, principal, id, model.ActionAcademicUnitManage, metadata,
	)
	if appErr != nil {
		return nil, appErr
	}
	if patch == nil {
		return nil, invalidAdministrationRequest("PatchAcademicUnit", "patch")
	}
	current, err := a.Store().AcademicUnit().Get(ctx, id)
	if err != nil {
		return nil, administrationError("PatchAcademicUnit.get", "academic_unit", err)
	}
	candidate := *current
	candidate.Patch(patch)
	if patch.ParentId != nil && candidate.ParentId != "" && candidate.ParentId != current.ParentId {
		if _, appErr := a.authorizePrincipalToAcademicUnit(
			ctx, principal, candidate.ParentId, model.ActionAcademicUnitManage, metadata,
		); appErr != nil {
			return nil, appErr
	...
	attempt, appErr := a.beginAdministrationMutation(
		ctx, principal, model.ActionAcademicUnitManage, resource, metadata,
		"patch", candidate.Auditable(), current.Auditable(),
	)
	if appErr != nil {
		return nil, appErr
	}
	updated, err := a.Store().AcademicUnit().Update(ctx, &candidate)
	if err != nil {
		return nil, a.failAdministrationMutation(
			ctx, attempt.Id, "PatchAcademicUnit", "academic_unit", err,
		)
	}
	if appErr := a.completeAdministrationMutation(ctx, attempt.Id, updated); appErr != nil {
		return nil, appErr
...
func (a *App) ArchiveAcademicUnit(
	...
	current, err := a.Store().AcademicUnit().Get(ctx, id)
	if err != nil {
		return administrationError("ArchiveAcademicUnit.get", "academic_unit", err)
	}
	attempt, appErr := a.beginAdministrationMutation(
		ctx, principal, model.ActionAcademicUnitManage, resource, metadata,
		"archive", map[string]any{"id": id}, current.Auditable(),
	)
	if appErr != nil {
		return appErr
	}
	archived, err := a.Store().AcademicUnit().Delete(ctx, id, time.Now().UnixMilli())
	if err != nil {
		return a.failAdministrationMutation(
			ctx, attempt.Id, "ArchiveAcademicUnit", "academic_unit", err,
		)
	}
