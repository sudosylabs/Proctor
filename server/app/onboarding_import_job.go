...
func (h onboardingImportParseHandler) Run(ctx context.Context, execution jobengine.Execution) jobengine.Outcome {
	...
	if err = h.service.parse(ctx, model.OnboardingImportID(command.ImportID)); err != nil {
		if errors.Is(err, context.Canceled) {
			return jobengine.Canceled("job.canceled")
		}
		if failure, ok := As(err); ok && (failure.Code() == "authentication.invalid_token" || failure.Code() == "authorization.denied") {
			return jobengine.PermanentFailure("onboarding_import.authorization_lost", err)
		}
		if errors.Is(err, errOnboardingImportInvalidFile) {
			return jobengine.PermanentFailure("onboarding_import.invalid_file", err)
		}
		return jobengine.RetryableFailure("dependency.unavailable", err)
	}
	return jobengine.Outcome{Kind: jobengine.OutcomeSucceeded, ResultVersion: 1, Result: json.RawMessage(`{"preview_ready":true}`)}
...
func (h onboardingImportExecuteHandler) Run(ctx context.Context, execution jobengine.Execution) jobengine.Outcome {
	...
	current, err := h.service.imports.GetOnboardingImport(ctx, model.OnboardingImportID(command.ImportID))
	if err != nil {
		return jobengine.RetryableFailure("dependency.unavailable", err)
	}
