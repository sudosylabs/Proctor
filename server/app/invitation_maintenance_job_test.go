...
func TestInvitationMaintenanceJobConsumesBoundedPages(t *testing.T) {
	...
	outcome := (invitationMaintenanceHandler{invitations: persistence}).Run(context.Background(), jobengine.NewExecution(job, nil, nil, nil))
	...
	descriptor := invitationMaintenanceDescriptor(invitationMaintenanceHandler{invitations: persistence})
