...
func TestExamSittingSealingContinuationUseCaseCreatesPermanentDeduplicatedJob(t *testing.T) {
	t.Parallel()
	at := time.Date(2026, time.August, 18, 9, 0, 0, 0, time.UTC)
	sittingID, parentID := model.NewExamSittingID(), model.NewJobID()
	jobs := &deduplicatingJobEnqueuerFake{jobs: map[string]*model.Job{}}
	useCases := examSittingSealingJobUseCases{jobs: jobs, now: func() time.Time { return at }, newID: model.NewJobID}
	if err := useCases.EnqueueExamSittingSealingContinuationFromJob(context.Background(), sittingID, parentID); err != nil {
		t.Fatal(err)
	}
	if err := useCases.EnqueueExamSittingSealingContinuationFromJob(context.Background(), sittingID, parentID); err != nil {
		t.Fatal(err)
	...
	if len(jobs.jobs) != 1 {
		t.Fatalf("continuation jobs=%#v", jobs.jobs)
	}
	for _, child := range jobs.jobs {
		command, err := model.DecodeExamSittingLifecycleCommand(child.CommandVersion, child.Command)
		if err != nil || command.ExamSittingID != sittingID || child.Type != model.JobTypeExamSittingSealing ||
			child.DedupePolicy != model.JobDedupePermanent ||
			child.DedupeKey != "exam-sitting-sealing-continuation:"+parentID.String() ||
			!child.AvailableAt.Equal(at) {
			t.Fatalf("continuation=%#v command=%#v err=%v", child, command, err)
		}
	}
}
